...
from ..models.task import Category, Priority, Task
...
def create_task(
    ...
    exists_task = db.query(Task).filter(Task.name==task_data.name, Task.user_id==user.user_id).first()
...
@router.get("/")
def get_task_list(user: Annotated[User, Depends(get_user)], db: Annotated[Session, Depends(get_db)]):
...
def get_one_task(
    ...
    exists_task = db.query(Task).filter(Task.task_id==pk, Task.user_id==user.user_id).first()
...
def update_task(
    ...
    if task_data.name:
        exists_name = db.query(Task).filter(Task.name==task_data.name, Task.user_id==user.user_id).first()
        if exists_name:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Task with this name already exists."
    ...
    if task_data.category_id:
        exists_category = db.query(Category).filter(Category.category_id==task_data.category_id).first()
...
@router.delete("/{pk}")
def delete_task(pk: int, user: Annotated[User, Depends(get_user)], db: Annotated[Session, Depends(get_db)]):
    task = db.query(Task).filter(Task.task_id==pk, Task.user_id==user.user_id).first()
